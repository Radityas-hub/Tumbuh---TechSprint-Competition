...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await parseJsonBody(request, createMediaUploadSchema);
    await getOwnedChildForGuardian(guardian.id, body.childId);
    await assertConsentGrantedForChild(body.childId, "media_upload");

    const uploadRequest = await createUploadRequestForChild(body);
